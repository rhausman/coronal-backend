...
class RHRAD_online:
    def __init__(
        ...
        # Process data
        df1 = self.resting_heart_rate(
            self.fitbit_hr, self.fitbit_steps
        )  # RHR df at 1min resolution
        ...
        self.online_anomaly_detection(
            data_seasnCorec, self.baseline_window, self.sliding_window
        )
        ...
        if len(self.pos_anomalies) > 0:
            self.alerts = self.create_alerts(
                self.pos_anomalies, results, self.fitbit_hr
            )
    ...
    def online_anomaly_detection(
        self, data_seasnCorec, baseline_window, sliding_window
    ):
        """
    ...
    def create_alerts(self, anomalies, data, fitbit_oldProtocol_hr):
        ...
        daily_alerts = alerts_reset.resample(
            "24H", on="datetime", base=21, label="right"
        ).count()
        ...
        df3 = pd.merge(
            df_hr, daily_alerts, how="outer", left_index=True, right_index=True
        )
    ...
    def visualize(
        self, results, positive_anomalies, test_alerts, symptom_date, diagnosis_date
    ):
        """
...
class resultsProcesser:
    def __init__(self, anomaliesCSV, alertsCSV):
